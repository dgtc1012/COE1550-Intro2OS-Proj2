...
asmlinkage long sys_cs1550_down(struct cs1550_sem *sem) 
{
    ...
    sem->value--;
    if(sem->value < 0){
        //enqueue process in semaphore
        if(sem->start == NULL){
            sem->start = (struct Node *)kmalloc(sizeof(struct Node), GFP_KERNEL);
            sem->end = sem->start;
            sem->start->task = current;
            sem->start->next = NULL;
        }
        else{
            sem->end->next = (struct Node *)kmalloc(sizeof(struct Node), GFP_KERNEL);
            sem->end = sem->end->next;
            sem->end->next = NULL;
            sem->end->task = current;
        }
...
asmlinkage long sys_cs1550_up(struct cs1550_sem *sem) 
{
    ...
    sem->value++;
    if(sem->value <= 0){
        //dequeue process from semaphore, then run
        if(sem->start==NULL){
            return -1;
        }
        else{
            task = sem->start->task;
            struct Node * temp = sem->start;
